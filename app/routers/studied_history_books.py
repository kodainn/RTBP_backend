...
async def list_studied_history_books(session: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
async def individual_studied_history_book(book_id: int, session: Session = Depends(get_db), user: User = Depends(get_current_user)):
