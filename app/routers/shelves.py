...
async def create_shelve(req_body: CreateShelve, session: Session = Depends(get_db), user = Depends(get_current_user)):
    shelve = ShelveService(session, user).create(req_body)
    return shelve


@router.patch("/shelves/{id}", response_model=OutputShelve)
async def update_shelve(id: int, req_body: UpdateShelve, session: Session = Depends(get_db), user = Depends(get_current_user)):
    shelve = ShelveService(session, user).update(id, req_body)
    return shelve


@router.delete("/shelves/{id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_shelve(id: int, session: Session = Depends(get_db), user = Depends(get_current_user)):
