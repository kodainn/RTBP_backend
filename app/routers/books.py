...
async def individual_book(id: int, user: User = Depends(get_current_user), session: Session = Depends(get_db)):
...
async def create_book(req_body: CreateBook, user: User = Depends(get_current_user), session: Session = Depends(get_db)):
...
async def update_book(id: int, req_body: UpdateBook, user: User = Depends(get_current_user), session: Session = Depends(get_db)):
...
async def delete_book(id: int, user: User = Depends(get_current_user), session: Session = Depends(get_db)):
