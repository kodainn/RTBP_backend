...
async def individual_book(id: int, session: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
async def create_book(req_body: CreateBook, session: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
async def update_book(id: int, req_body: UpdateBook, session: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
async def delete_book(id: int, session: Session = Depends(get_db), user: User = Depends(get_current_user)):
